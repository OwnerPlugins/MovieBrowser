...
echo "Starting MovieBrowser installation..."
...
cleanup() {
    ...
    [ -d "/tmp/MovieBrowser-main" ] && rm -rf "/tmp/MovieBrowser-main"
}
...
if [ $? -ne 0 ]; then
    echo "Failed to extract MovieBrowser package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/MovieBrowser-main/usr/lib/enigma2/python/Plugins/Extensions/MovieBrowser" ]; then
    cp -r "$TMPPATH/MovieBrowser-main/usr/lib/enigma2/python/Plugins/Extensions/MovieBrowser"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/MovieBrowser-main/usr/lib64/enigma2/python/Plugins/Extensions/MovieBrowser" ]; then
    cp -r "$TMPPATH/MovieBrowser-main/usr/lib64/enigma2/python/Plugins/Extensions/MovieBrowser"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/MovieBrowser-main/usr" ]; then
    cp -r "$TMPPATH/MovieBrowser-main/usr"/* /usr/ 2>/dev/null
